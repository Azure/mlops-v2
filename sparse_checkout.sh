...
git_folder_location='/Users/ritajasengupta/work-git/generated/'   #replace with the local root folder location where you want to create the project folder
project_name=ml-classical-fs   #replace with your project name
github_org_name=ritaja   #replace with your github org name
project_template_github_url=https://github.com/ritaja/mlops-project-template  #replace with the url for the project template for your organization created in step 2.2, or leave for demo purposes

cd $git_folder_location

# Clone MLOps Project repo
git clone \
  --branch 'feature/featurestore_tf' \
  --depth 1  \
  --filter=blob:none  \
  --sparse \
